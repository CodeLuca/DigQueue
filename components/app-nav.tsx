...
function itemClass(active: boolean) {
  return active
    ? "inline-flex min-h-9 shrink-0 items-center justify-center gap-1.5 rounded-md border border-[var(--color-accent)] bg-[color-mix(in_oklab,var(--color-accent)_20%,var(--color-surface2)_80%)] px-2.5 py-1.5 text-center text-xs font-medium leading-tight text-[var(--color-text)] whitespace-nowrap shadow-[0_0_0_1px_color-mix(in_oklab,var(--color-accent)_45%,transparent)] sm:px-3 sm:text-sm"
    : "inline-flex min-h-9 shrink-0 items-center justify-center gap-1.5 rounded-md border border-[var(--color-border)] bg-[var(--color-surface)]/35 px-2.5 py-1.5 text-center text-xs leading-tight text-[var(--color-muted)] whitespace-nowrap hover:bg-[var(--color-surface2)] hover:text-[var(--color-text)] sm:px-3 sm:text-sm";
}
  ...
  return (
    <div className="sticky top-0 z-40 border-b border-[var(--color-border-soft)] bg-[color-mix(in_oklab,var(--color-bg)_88%,black_12%)]/95 backdrop-blur">
      <div className="mx-auto flex max-w-[1400px] flex-col gap-2 px-4 py-2 md:flex-row md:items-center md:justify-between md:gap-4 md:px-8">
        ...
        <nav className="flex w-full items-center gap-2 overflow-x-auto rounded-lg border border-[var(--color-border-soft)] bg-[var(--color-surface)]/30 p-2 [scrollbar-width:none] [-ms-overflow-style:none] [&::-webkit-scrollbar]:hidden md:w-auto md:flex-nowrap md:justify-end" aria-label="Primary tabs">
