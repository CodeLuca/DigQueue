  ...
  try {
    ...
    const scopedTotals = await Promise.all([
      needsUnplayedTracksMetric
        ? db
            ...
            .innerJoin(sourceReleases, eq(sourceReleases.releaseId, releases.id))
            .innerJoin(labels, eq(sourceReleases.sourceId, labels.id))
            .where(and(eq(tracks.listened, false), eq(labels.active, true), scope.tracks, scope.releases, scope.sourceReleases, scope.labels))
        : Promise.resolve([{ value: 0 }]),
      needsPlayedItemsMetric
        ? db
            ...
            .innerJoin(sourceReleases, eq(sourceReleases.releaseId, releases.id))
            .innerJoin(labels, eq(sourceReleases.sourceId, labels.id))
            .where(and(eq(queueItems.status, "played"), eq(labels.active, true), scope.queueItems, scope.releases, scope.sourceReleases, scope.labels))
        : Promise.resolve([{ value: 0 }]),
      needsDoneTracksMetric
        ? db
            ...
            .innerJoin(sourceReleases, eq(sourceReleases.releaseId, releases.id))
            .innerJoin(labels, eq(sourceReleases.sourceId, labels.id))
            .where(and(eq(tracks.listened, true), eq(labels.active, true), scope.tracks, scope.releases, scope.sourceReleases, scope.labels))
        : Promise.resolve([{ value: 0 }]),
      needsSavedTracksMetric
        ? db
            ...
            .innerJoin(sourceReleases, eq(sourceReleases.releaseId, releases.id))
            .innerJoin(labels, eq(sourceReleases.sourceId, labels.id))
            .where(and(eq(tracks.saved, true), eq(labels.active, true), scope.tracks, scope.releases, scope.sourceReleases, scope.labels))
        : Promise.resolve([{ value: 0 }]),
      needsWishlistMetric
        ? db
            .select({ value: count() })
            ...
            .innerJoin(sourceReleases, eq(sourceReleases.releaseId, releases.id))
            .innerJoin(labels, eq(sourceReleases.sourceId, labels.id))
            .where(and(eq(releases.wishlist, true), eq(labels.active, true), scope.releases, scope.sourceReleases, scope.labels))
        : Promise.resolve([{ value: 0 }]),
    ]);
    ...
    const lowConfidenceCount = needsErrorSummary
      ? await db
          ...
          .innerJoin(sourceReleases, eq(sourceReleases.releaseId, releases.id))
          .innerJoin(labels, eq(sourceReleases.sourceId, labels.id))
          .where(and(eq(releases.youtubeMatched, true), lt(releases.matchConfidence, 0.4), eq(labels.active, true), scope.releases, scope.sourceReleases, scope.labels))
      : [{ value: 0 }];
    ...
    try {
      const userId = await requireCurrentAppUserId();
      const scope = userScope(userId);
      const legacyLabelRows = await db.query.labels.findMany({
        where: and(eq(labels.sourceType, "workspace"), scope.labels),
        orderBy: [asc(labels.addedAt)],
      });
      const releaseCountRows = await db
        .select({ labelId: releases.labelId, value: count() })
        .from(releases)
        .where(scope.releases)
        .groupBy(releases.labelId);
      const fetchedReleaseRows = await db
        .select({ labelId: releases.labelId, value: count() })
        .from(releases)
      ...
      const labelsWithMetadata = legacyLabelRows.map((label) => {
        const loadedReleaseCount = releaseCountByLabel.get(label.id) ?? 0;
        const fetchedReleaseCount = fetchedReleaseCountByLabel.get(label.id) ?? 0;
        const loadedTrackCount = trackCountByLabel.get(label.id) ?? 0;
        const pagesDone = label.currentPage > Math.max(1, label.totalPages);
...
export async function getLabelDetail(labelId: number) {
  ...
  let labelReleases: Awaited<ReturnType<typeof db.query.releases.findMany>> = [];
  let counts: Array<Array<{ value: number }>> = [];
  try {
    ...
    labelReleases = labelMappings.map((item) => item.release).filter((item): item is NonNullable<typeof item> => Boolean(item));
    ...
    labelReleases = await db.query.releases.findMany({
...
export async function getToListenData(labelId?: number, onlyPlayable = true) {
  ...
  try {
    const whereClause = labelId
      ? and(or(eq(tracks.listened, false), eq(tracks.saved, true)), eq(sourceReleases.sourceId, labelId), eq(labels.active, true), scope.tracks, scope.releases, scope.sourceReleases, scope.labels)
      : and(or(eq(tracks.listened, false), eq(tracks.saved, true)), eq(labels.active, true), scope.tracks, scope.releases, scope.sourceReleases, scope.labels);
...
export async function getPlayedReviewedData(labelId?: number, onlyPlayable = false) {
  ...
  const whereClause = labelId
    ? and(or(eq(tracks.listened, true), inArray(tracks.id, playedTrackIds.length ? playedTrackIds : [-1])), eq(sourceReleases.sourceId, labelId), eq(labels.active, true), scope.tracks, scope.releases, scope.sourceReleases, scope.labels)
    : and(or(eq(tracks.listened, true), inArray(tracks.id, playedTrackIds.length ? playedTrackIds : [-1])), eq(labels.active, true), scope.tracks, scope.releases, scope.sourceReleases, scope.labels);
  const playableClause = onlyPlayable ? isNotNull(youtubeMatches.id) : undefined;
  const combinedWhere = playableClause ? and(whereClause, playableClause, scope.youtubeMatches) : whereClause;
...
export async function getWishlistedRecordsData(labelId?: number) {
  ...
  try {
    const whereClause = labelId
      ? and(eq(releases.wishlist, true), eq(sourceReleases.sourceId, labelId), scope.releases, scope.sourceReleases, scope.labels)
      : and(eq(releases.wishlist, true), scope.releases, scope.sourceReleases, scope.labels);
