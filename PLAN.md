...
3. Regression coverage phase 2: route-level tests for OAuth start/callback handlers and queue playback state transitions (OAuth provider/start/redirect/callback query/messages/url + temp-cookie options/keys mapping + queue mutation/selection helpers are now extracted and covered).
