...
## Next Iterations
1. Failure center phase 2: expand remediation actions per category beyond current retry/reconnect/lock-cleanup/metadata-refresh/rate-limit-pause controls.
2. Processing observability phase 2: longer-window trend views (run duration percentiles + 10-minute timeline with bar visualization + 60-minute summary are now surfaced).
3. Regression coverage phase 2: route-level tests for OAuth start/callback handlers and queue playback state transitions (OAuth start/redirect/callback query helpers + queue mutation input/feedback helpers are now extracted and covered).
4. Copy and mobile UX polish pass across all tabs (Failure Center + sync status density reduced; continue simplifying labels/actions on narrow screens).
