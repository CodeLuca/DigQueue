...
3. Regression coverage phase 2: route-level tests for OAuth start/callback handlers and queue playback state transitions (OAuth start/redirect/callback query + temp-cookie + callback-login helpers and queue mutation/selection helpers are now extracted and covered).
